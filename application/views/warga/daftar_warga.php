				...
				<tbody>
					...
					<tr>
						...
						<td><?php if($this->session->userdata('role')!='Ketua'){} else{ ?><a href="<?php echo site_url('Warga/profil/'.$a->no_ktp);?>" class="btn bg-green waves-effect"><i class="material-icons">visibility</i></a><?php if($a->role =='Ketua' or $a->role == 'Bendahara' or $a->role =='Sekretaris'){ ?>
                         ...
                         <?php }else{ ?> <?php if(get_pengurus_rt($a->rt,'Bendahara')->num_rows() > 0){} else { ?>
                            <a href="<?php echo site_url('Panel_rt/jadikan_bendahara/'.$a->id_user);?>" class="btn bg-blue">Jadikan Bendahara</a>
                            <?php } ?>
                            <?php if(get_pengurus_rt($a->rt,'Sekretaris')->num_rows() > 0){} else{?>
                             <a href="<?php echo site_url('Panel_rt/jadikan_sekretaris/'.$a->id_user);?>" class="btn bg-orange">Jadikan Sekretaris</a><?php }} }?></td>
